...
import { getConcept } from "@/i18n/concepts.server";
...
export async function renderOgImage(slug: string, locale: Locale) {
  const concept = getConcept(slug, locale)?.content;
  const messages = getUiMessages(locale);
  const staticPage = PAGES[slug];
  const page = concept ?? (staticPage ? {
    title: slug === "goats" ? messages.goats : messages.resources,
    description: pageCopy(locale, staticPage.description),
  } : undefined);
  const title = page?.title ?? SITE_NAME;
  const description = page?.description ?? messages.siteDescription;
